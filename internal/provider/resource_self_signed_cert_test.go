...
func TestSelfSignedCert(t *testing.T) {
	r.UnitTest(t, r.TestCase{
		...
		Steps: []r.TestStep{
			{
				Config: selfSignedCertConfig(1, 0, "RSA", testPrivateKey),
			...
			{
				Config: selfSignedCertConfig(1, 0, "PKCS#8", testPrivatePKCS8Key),
				Check: func(s *terraform.State) error {
					gotUntyped := s.RootModule().Outputs["key_pem_1"].Value
					got, ok := gotUntyped.(string)
					if !ok {
...
func TestAccSelfSignedCertRecreatesAfterExpired(t *testing.T) {
	...
	r.UnitTest(t, r.TestCase{
		...
		Steps: []r.TestStep{
			{
				Config: selfSignedCertConfig(10, 2, "RSA", testPrivateKey),
				Check: func(s *terraform.State) error {
					gotUntyped := s.RootModule().Outputs["key_pem_1"].Value
					got, ok := gotUntyped.(string)
					if !ok {
...
func TestAccSelfSignedCertNotRecreatedForEarlyRenewalUpdateInFuture(t *testing.T) {
	...
	r.UnitTest(t, r.TestCase{
		...
		Steps: []r.TestStep{
			...
			{
				Config: selfSignedCertConfig(10, 3, "RSA", testPrivateKey),
				Check: func(s *terraform.State) error {
					gotUntyped := s.RootModule().Outputs["key_pem_1"].Value
					got, ok := gotUntyped.(string)
					if !ok {
			...
			{
				PreConfig: setTimeForTest("2019-06-14T16:00:00Z"),
				Config:    selfSignedCertConfig(10, 9, "RSA", testPrivateKey),
				Check: func(s *terraform.State) error {
					gotUntyped := s.RootModule().Outputs["key_pem_1"].Value
					got, ok := gotUntyped.(string)
					if !ok {
...
func selfSignedCertConfig(validity uint32, earlyRenewal uint32, algo string, privateKey string) string {
	return fmt.Sprintf(`
...
EOT
                ...
                `, validity, earlyRenewal, algo, privateKey)
}
