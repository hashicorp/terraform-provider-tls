...
- `private_key_pem_wo` (String, Sensitive, Write-only) Private key in [PEM (RFC 1421)](https://datatracker.ietf.org/doc/html/rfc1421) format, that the certificate will belong to. This attribute is write-only and will not be stored in the state.
- `private_key_pem_wo_version` (String) Version of the private key. If this changes, the certificate will be regenerated.
