...
- `private_key_pem_wo` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) Write only variant of private_key_pem. Either private_key_pem_wo or private_key_pem must be provided.
- `private_key_pem_wo_version` (Number) Triggers update of the certificate request using the private_key_pem_wo write-only value.
